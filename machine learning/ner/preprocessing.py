...
def build_vocabulary(path_data,path_vocs_dict,min_counts_dict,columns):
    ...
    for i,name in enumerate(columns):
        size = create_dictionary(feature_item_dict_list[i],path_vocs_dict[name],
                                 start=1,sort=True,min_count=min_counts_dict[name],
                                 overwrite=True)
...
def main():
    ...
    voc_sizes,sequence_length = build_vocabulary(path_data=config['data_params']['path_train'],
                                                 columns=columns,
    ...
    for i,feature_name in enumerate(feature_names):
        if i == 0:
            config['model_params']['embed_params'][feature_name]['shape'] = [voc_sizes[i]+1, feature_dim_dict[feature_name]]
        else:
            config['model_params']['embed_params'][feature_name]['shape'] = [voc_sizes[i] + 1, feature_dim_dict[feature_name]]
